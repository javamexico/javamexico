...
CREATE TABLE usuario(
	uid        INTEGER DEFAULT nextval('usuario_uid_seq') PRIMARY KEY,
...
create sequence tag_usuario_tid_seq;
CREATE TABLE tag_usuario(
	tid   integer default nextval('tag_usuario_tid_seq') PRIMARY KEY,
...
create sequence tema_foro_tid_seq;
CREATE TABLE tema_foro(
	tid  integer default nextval('tema_foro_tid_seq') PRIMARY KEY,
...
create sequence foro_fid_seq;
CREATE TABLE foro(
	fid integer default nextval('foro_fid_seq') PRIMARY KEY,
...
create sequence tag_foro_tid_seq;
CREATE TABLE tag_foro(
	tid   integer default nextval('tag_foro_tid_seq') PRIMARY KEY,
...
create sequence coment_foro_cfid_seq;
CREATE TABLE coment_foro(
	cfid integer default nextval('coment_foro_cfid_seq') PRIMARY KEY,
...
create sequence voto_foro_vid_seq;
CREATE TABLE voto_foro(
	vid   integer default nextval('voto_foro_vid_seq') PRIMARY KEY,
	fid   INTEGER NOT NULL REFERENCES foro(fid) ON DELETE CASCADE,
	uid   INTEGER NOT NULL REFERENCES usuario(uid) ON DELETE CASCADE,
	fecha TIMESTAMP NOT NULL,
...
create sequence voto_coment_foro_vid_seq;
CREATE TABLE voto_coment_foro(
	vid   integer default nextval('voto_coment_foro_vid_seq') PRIMARY KEY,
...
create sequence pregunta_pid_seq;
CREATE TABLE pregunta(
	pid      integer default nextval('pregunta_pid_seq') PRIMARY KEY,
	uid      INTEGER NOT NULL REFERENCES usuario(uid) ON DELETE CASCADE,
...
create sequence respuesta_rid_seq;
CREATE TABLE respuesta(
	rid   integer default nextval('respuesta_rid_seq') PRIMARY KEY,
...
create sequence coment_pregunta_cid_seq;
CREATE TABLE coment_pregunta(
	cid    integer default nextval('coment_pregunta_cid_seq') PRIMARY KEY,
	pid    INTEGER NOT NULL REFERENCES pregunta(pid) ON DELETE CASCADE,
	uid    INTEGER NOT NULL REFERENCES usuario(uid) ON DELETE CASCADE,
	fecha  TIMESTAMP NOT NULL,
...
create sequence coment_respuesta_cid_seq;
CREATE TABLE coment_respuesta(
	cid    integer default nextval('coment_respuesta_cid_seq') PRIMARY KEY,
...
create sequence tag_pregunta_tid_seq;
CREATE TABLE tag_pregunta(
	tid   integer default nextval('tag_pregunta_tid_seq') PRIMARY KEY,
	tag   VARCHAR(80) NOT NULL UNIQUE
);
...
create sequence voto_pregunta_vid_seq;
CREATE TABLE voto_pregunta(
	vid   integer default nextval('voto_pregunta_vid_seq') PRIMARY  KEY,
	pid   INTEGER NOT NULL REFERENCES pregunta(pid) ON DELETE CASCADE,
	uid   INTEGER NOT NULL REFERENCES usuario(uid) ON DELETE CASCADE,
	fecha TIMESTAMP NOT NULL,
	up    BOOLEAN NOT NULL DEFAULT true
...
create sequence voto_respuesta_vid_seq;
CREATE TABLE voto_respuesta(
	vid   integer default nextval('voto_respuesta_vid_seq') PRIMARY  KEY,
	rid   INTEGER NOT NULL REFERENCES respuesta(rid) ON DELETE CASCADE,
	uid   INTEGER NOT NULL REFERENCES usuario(uid) ON DELETE CASCADE,
	fecha TIMESTAMP NOT NULL,
	up    BOOLEAN NOT NULL DEFAULT true
...
CREATE TABLE blog_coment(
	...
	uid   INTEGER NOT NULL REFERENCES usuario(uid)  ON DELETE CASCADE,
	rt    INTEGER REFERENCES blog_coment(cid) ON DELETE RESTRICT, --nullify
	fecha TIMESTAMP NOT NULL,
	coment VARCHAR(2000) NOT NULL
);
